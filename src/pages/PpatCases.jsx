...
export default function PpatCases() {
  ...
  const currentServiceConfig = useMemo(() => {
    return PPAT_SERVICES.find((s) => s.id === form.serviceType) || PPAT_SERVICES[0];
  }, [form.serviceType]);

  const myCases = useMemo(() => cases.filter(c => PPAT_SERVICES.some(s => s.id === c.serviceType)), [cases]);
  ...
  const handleServiceChange = (st) => {
    const s = PPAT_SERVICES.find((serv) => serv.id === st);
